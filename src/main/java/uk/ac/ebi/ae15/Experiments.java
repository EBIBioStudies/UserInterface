...
//          " where i.identifier like 'E-GEOD-2%'" +
            " order by" +
            "  i.identifier asc";
...
class GetExperimentXmlThread extends Thread implements Runnable {
    ...
    private static String getExperimentXmlSql = "select XmlElement( \"experiment\"" +
        ...
        " , ( select /*+ index(ed) */ XmlAgg( XmlElement( \"experimentdesign\", translate(replace(oe.value,'_design',''),'_',' ') ) ) from tt_experimentdesign ed, tt_types_t_experimentdesign tte, tt_ontologyentry oe where ed.t_experiment_id = e.id and tte.t_experimentdesign_id = ed.id and oe.id = tte.types_id and oe.category = 'ExperimentDesignType' )" +
