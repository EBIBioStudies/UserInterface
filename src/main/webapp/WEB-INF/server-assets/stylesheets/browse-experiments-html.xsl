...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/experiments">
        <helper:logDebug
                select="Parameters: keywords [{$keywords}], wholewords [{$wholewords}], array [{$array}], species [{$species}], detailedview [{$detailedview}]"/>
        <helper:logDebug select="Sort by: [{$sortby}], [{$sortorder}]"/>
        <xsl:variable name="vFilteredExperiments"
                      select="ae:filter-experiments($keywords,$wholewords,$species,$array)"/>
        ...
        <xsl:variable name="vFrom">
            <xsl:choose>
                <xsl:when test="$page &gt; 0">
                    <xsl:value-of select="1 + ( $page - 1 ) * $pagesize"/>
                </xsl:when>
        ...
        <xsl:variable name="vTo">
            <xsl:choose>
                <xsl:when test="( $vFrom + $pagesize - 1 ) &gt; $vTotal">
                    <xsl:value-of select="$vTotal"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$vFrom + $pagesize - 1"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>

        <xsl:variable name="vDetailedViewExtStyle">
            <xsl:if test="not($detailedview)">display:none</xsl:if>
        </xsl:variable>
        <xsl:variable name="vDetailedViewMainClass">ae_results_tr_main
            <xsl:if test="$detailedview">tr_main_expanded</xsl:if>
        </xsl:variable>

        <helper:logInfo
                select="Query for '{$keywords}' filtered {$vTotal} experiments. Will output from {$vFrom} to {$vTo}."/>
        ...
        <tr id="ae_results_summary_info">
            <td colspan="8">
                ...
                <div id="ae_results_total_samples">
                    <xsl:value-of select="$vTotalSamples"/>
                </div>
                <div id="ae_results_total_assays">
                    <xsl:value-of select="$vTotalAssays"/>
                </div>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <tr class="ae_results_tr_error">
                    <td colspan="8">
                        <p>
                            <strong>The query '<xsl:value-of select="$keywords"/>'
                        ...
                        <p>Try shortening the query term e.g. 'embryo' will match embryo, embryoid, embryonic across all
                            annotation fields.
                        </p>
                        <p>Note that '*' is
                            <strong>not</strong>
                            supported as a wild card. More information available at<a
                                href="http://www.ebi.ac.uk/microarray/doc/help/ae_help.html">ArrayExpress Query Help</a>
                            .
                        </p>
                    </td>
                </tr>
    ...
    <xsl:template match="experiment">
        ...
        <xsl:if test="position() &gt;= $pFrom and position() &lt;= $pTo">
            <tr id="{$vExpId}_main" class="{$pDetailedViewMainClass}">
                <td>
                    <div class="table_row_expand">&#160;</div>
                </td>
                <td>
                    <div>
                ...
                <td>
                    <div><xsl:apply-templates select="name" mode="highlight"/>&#160;
                    </div>
                </td>
                <td class="align_right">
                    <div><xsl:apply-templates select="assays" mode="highlight"/>&#160;
                    </div>
                </td>
                <td>
                    <div>
                ...
                <td>
                    <div><xsl:apply-templates select="releasedate" mode="highlight"/>&#160;
                    </div>
                </td>
                <td class="align_center">
                    <div>
                        <xsl:choose>
                            <xsl:when test="helper:isFileAvailableForDownload(files/fgem/@name)">
                                <a href="{concat('${interface.application.base.url}/download/',files/fgem/@name)}"
                                   title="Click to download processed data">
                                    <img src="${interface.application.base.url}/assets/images/silk_data_save.gif"
                                         width="16" height="16" alt="Click to download processed data"/>
                                </a>
                            </xsl:when>
                            <xsl:otherwise>
                                <img src="${interface.application.base.url}/assets/images/silk_data_unavail.gif"
                                     width="16" height="16"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </div>
                ...
                <td class="align_center">
                    <div>
                        <xsl:choose>
                            <xsl:when
                                    test="helper:isFileAvailableForDownload(files/raw/@name) and files/raw/@celcount>0">
                                <a href="{concat('${interface.application.base.url}/download/',files/raw/@name)}"
                                   title="Click to download Affymetrix data">
                                    <img src="${interface.application.base.url}/assets/images/silk_data_save_affy.gif"
                                         width="16" height="16" alt="Click to download Affymetrix data"/>
                                </a>
                            </xsl:when>
                            <xsl:when
                                    test="helper:isFileAvailableForDownload(files/raw/@name) and files/raw/@celcount=0">
                                <a href="{concat('${interface.application.base.url}/download/',files/raw/@name)}"
                                   title="Click to download raw data">
                                    <img src="${interface.application.base.url}/assets/images/silk_data_save.gif"
                                         width="16" height="16" alt="Click to download raw data"/>
                                </a>
                            </xsl:when>
                            <xsl:otherwise>
                                <img src="${interface.application.base.url}/assets/images/silk_data_unavail.gif"
                ...
                <td colspan="8">
                    <table class="ae_results_tr_ext_table" cellpadding="0" cellspacing="0" border="0">
                        ...
                        <tr style="${interface.application.link.aer_old.experiment_link.style}">
                            ...
                            <td class="value">
                                <a href="${interface.application.link.aer_old.base.url}/result?queryFor=Experiment&amp;eAccession={accession/text()}"
                                   target="_blank" title="Opens in a new window">&#187; Advanced interface page for the
                                    experiment
                                </a>
                            </td>
                        </tr>
                        <xsl:if test="count(secondaryaccession/text())&gt;0">
                            <tr>
                                <td class="name">Secondary&#160;accession<xsl:if
                                        test="count(secondaryaccession/text())&gt;1">s</xsl:if>:
                                </td>
                                <td class="value">
                        ...
                        <xsl:if test="miamescores">
                            <tr>
                                ...
                                <td class="value">
                                    <strong>
                                        <xsl:value-of select="miamescores/overallscore"/>
                                    </strong>
                                ...
                                <xsl:choose>
                                    <xsl:when test="helper:isFileAvailableForDownload(files/twocolumns/@name)">
                                        <a href="{concat('${interface.application.base.url}/download/',files/twocolumns/@name)}"
                                           target="_blank" title="Opens in a new window">&#187; Tab-delimited
                                            spreadsheet
                                        </a>
                                    </xsl:when>
                        ...
                        <xsl:if test="count(arraydesign)&gt;0">
                            <tr>
                                <td class="name">Array<xsl:if test="count(arraydesign)&gt;1">s</xsl:if>:
                                </td>
                                <td class="value">
                        ...
                        <xsl:if test="count(bibliography/*)&gt;0">
                            <tr>
                                <td class="name">Citation<xsl:if test="count(bibliography/*)&gt;1">s</xsl:if>:
                                </td>
                                <td class="value">
                        ...
                        <xsl:if test="count(provider[role!='data_coder'])&gt;0">
                            <tr>
                                <td class="name">Contact<xsl:if test="count(provider[role!='data_coder'])&gt;1">
                                    s</xsl:if>:
                                </td>
                                <td class="value">
                        ...
                        <xsl:if test="count(experimentdesign)&gt;0">
                            <tr>
                                <td class="name">Design&#160;type<xsl:if test="count(experimentdesign)&gt;1">s</xsl:if>:
                                </td>
                                <td class="value">
                        ...
                        <xsl:if test="count(description[text/text()!='' and not(contains(text/text(),'Generated description'))])&gt;0">
                            <tr>
                                ...
                                <td class="value">
                                    <xsl:for-each
                                            select="description[text/text()!='' and not(contains(text/text(),'Generated description'))]">
                        ...
                        <xsl:if test="count(experimentalfactor/name)&gt;0">
                            <tr>
                                <td class="name">Experimental factor<xsl:if test="count(experimentalfactor/name)&gt;1">
                                    s</xsl:if>:
                                </td>
                                <td class="attrs">
                        ...
                        <xsl:if test="count(sampleattribute/category)&gt;0">
                            <tr>
                                <td class="name">Sample attribute<xsl:if test="count(sampleattribute/category)&gt;1">
                                    s</xsl:if>:
                                </td>
                                <td class="attrs">
                                    <table cellpadding="0" cellspacing="2" border="0">
    ...
    <xsl:template match="bibliography">
        <p>
            <xsl:variable name="publication_title">
                <xsl:if test="authors/text()!=''"><xsl:apply-templates select="authors" mode="highlight"/>.
                </xsl:if>
                <xsl:if test="title/text()!=''"><xsl:apply-templates select="title" mode="highlight"/>.
                </xsl:if>
                <xsl:if test="publication/text()!=''">
                    <em>
                        <xsl:apply-templates select="publication" mode="highlight"/>
                    </em>
                    &#160;
                </xsl:if>
                <xsl:if test="volume/text()!=''">
                    <xsl:apply-templates select="volume" mode="highlight"/>
                    <xsl:if test="issue/text()!=''">(<xsl:apply-templates select="issue" mode="highlight"/>)
                    </xsl:if>
                </xsl:if>
                <xsl:if test="pages/text()!=''">:
                    <xsl:apply-templates select="pages" mode="highlight"/>
                </xsl:if>
                <xsl:if test="year/text()!=''">&#160;(<xsl:apply-templates select="year" mode="highlight"/>)
                </xsl:if>
                <xsl:if test="publication/text()!=''">.</xsl:if>
            </xsl:variable>
            <xsl:choose>
                <xsl:when test="uri[starts-with(., 'http')]">
                    <a href="{uri}" target="_blank" title="Opens in a new window">&#187;
                        <xsl:copy-of select="$publication_title"/>
                    </a>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:copy-of select="$publication_title"/>
                    <xsl:if test="uri/text()!=''">(<xsl:apply-templates select="uri" mode="highlight"/>)
                    </xsl:if>
                </xsl:otherwise>
            </xsl:choose>
            <xsl:if test="accession">
                <xsl:if test="number(accession)>0">(
                    <a href="http://www.ncbi.nlm.nih.gov/pubmed/{accession}" target="_blank"
                       title="Opens in a new window">&#187; PubMed
                        <xsl:apply-templates select="accession" mode="highlight"/>
                    </a>
                    )
                </xsl:if>
            ...
            <xsl:choose>
                ...
                <xsl:when test="substring(text(), 1, 3)='GSE' or substring(text(), 1, 3)='GDS'">
                    <a href="http://www.ncbi.nlm.nih.gov/projects/geo/query/acc.cgi?acc={text()}"
                       target="_blank" title="Opens in a new window">&#187; GEO
                        <xsl:apply-templates select="." mode="highlight"/>
                    </a>
                </xsl:when>
    ...
    <xsl:template name="providers">
        <xsl:for-each select="provider[not(contact=following-sibling::provider/contact) and role!='data_coder']">
            ...
            <xsl:apply-templates select="contact" mode="highlight"/>
            <xsl:if test="position()!=last()">,</xsl:if>
        </xsl:for-each>
    </xsl:template>
                ...
                <p>
                    <xsl:call-template name="add_highlight_element">
                        <xsl:with-param name="text"
                                        select="helper:markKeywords(substring-before($text, '&lt;br&gt;'),$keywords,$wholewords)"/>
                    </xsl:call-template>
                </p>
                <xsl:call-template name="description">
