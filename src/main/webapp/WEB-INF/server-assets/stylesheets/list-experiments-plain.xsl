<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="list-data">
        <xsl:param name="node"/>
        <xsl:choose>
            <xsl:when test="helper:isFileAvailableForDownload($node/@name)"><xsl:value-of select="concat('http://www.ebi.ac.uk/microarray-as/aer/download/mageml/',$node/@name)"/></xsl:when>
