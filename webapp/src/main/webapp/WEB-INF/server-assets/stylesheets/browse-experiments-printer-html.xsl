...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:html="http://www.w3.org/1999/xhtml" extension-element-prefixes="aejava search html"
	exclude-result-prefixes="aejava search html" version="2.0">

	<xsl:param name="sortby" />
	<xsl:param name="sortorder" />

	<xsl:variable name="vSortBy"
		select="if ($sortby) then $sortby else 'releasedate'" />
			...
			<xsl:if test="'true' = $initial">
				<xsl:call-template name="page-header-plain">
					...
					<xsl:with-param name="pExtraCode">
						<link rel="stylesheet" href="{$basepath}/assets/stylesheets/ae_common_20.css"
							type="text/css" />
						<link rel="stylesheet"
							href="{$basepath}/assets/stylesheets/ae_browse_printer_20.css"
							type="text/css" />
						...
						<script src="{$basepath}/assets/scripts/ae_common_20.js"
							type="text/javascript" />
						<script src="{$basepath}/assets/scripts/ae_browse_printer_20.js"
							type="text/javascript" />
					</xsl:with-param>
				</xsl:call-template>
			</xsl:if>
	...
	<xsl:template name="ae-contents">
		...
		<div class="ae_left_container_100pc">
			<xsl:if test="'true' = $initial">
			<div id="ae_header">
				<img src="{$basepath}/assets/images/ae_header.gif" alt="ArrayExpress" />
			</div>
			</xsl:if>
			<!-- <xsl:if test="'true' = $initial"> -->
			<xsl:choose>
				...
				<xsl:if test="'true' = $initial">
					<div id="ae_results_header">
						...
						<span id="ae_print_controls" class="noprint">
							<a href="javascript:window.print()">
								<img src="{$basepath}/assets/images/silk_print.gif" width="16"
									height="16" alt="Print" />
								Print this window
							</a>
							.
				...
				<xsl:otherwise>
					<div id="ae_infotext">
						<div>There are no experiments matching your search criteria found
							in ArrayExpress Archive.</div>
					</div>
				</xsl:otherwise>
			</xsl:choose>
		</div>
			...
			<td class="col_accession">
				<div>
					...
					<xsl:if test="not(user/@id = '1')">
						<img src="{$basepath}/assets/images/silk_lock.gif" width="8"
							height="9" />
					</xsl:if>
				</div>
			</td>
					...
					<xsl:choose>
						<xsl:when test="@loadedinatlas">
							<a href="${interface.application.link.atlas.exp_query.url}{accession}">
								<img src="{$basepath}/assets/images/basic_tick.gif" width="16"
									height="16" alt="*" />
							</a>
						</xsl:when>
						<xsl:otherwise>
							<img src="{$basepath}/assets/images/silk_data_unavail.gif"
	...
	<xsl:template name="data-files-main">
		...
		<xsl:choose>
			<xsl:when
				test="'0' != aejava:getAcceleratorValueAsString($pKind, $pAccession)">
				<img src="{$basepath}/assets/images/basic_tick.gif" width="16"
					height="16" alt="*" />
			</xsl:when>
			<xsl:otherwise>
