...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	extension-element-prefixes="fn html" exclude-result-prefixes="fn html"
	version="2.0">
...
<xsl:variable name="relative-uri"
	...
	<!-- <xsl:variable name="relative-referer" select="if (fn:starts-with($referer, 
		'/')) then fn:substring-after($referer, $context-path) else ''" /> <xsl:variable 
		name="secure-host" select="if (fn:matches($host, '^http[:]//www(dev)?[.]ebi[.]ac[.]uk$')) 
		then fn:replace($host, '^http[:]//', 'https://') else ''" /> -->
...
<nav>
							<ul class="grid_24" id="local-nav">
								...
								<li>
									<xsl:if test="fn:starts-with($relative-uri, '/browse.html') or fn:starts-with($relative-uri, '/group') or fn:starts-with($relative-uri, '/sample')">
