...
(function($) {
	...
	// to keep the fieldsnames of sample attributes (to file the autocomplete
	// for a scpecific samplegroup)
	var fieldsNames = [];
	...
	$.Autocompleter = function(input, options) {
		...
		function selectCurrent() {
			...
			do {
				...
				if (-1 != pos && pos <= cursorPos
						&& cursorPos <= pos + term.length) {
		...
		function getTermText(term) {
			...
			if (-1 != quotePos) {
				// alert("getTermText->"+substringBeforeFirst(term.substring(quotePos
				// + 1), "\""));
				return substringBeforeFirst(term.substring(quotePos + 1), "\"");
			} else {
				term = term.replace(/^[+-]/, "");
				// RPE:need to consider the [ and ]characters
				term = term.replace(/^[\w|\[|\]]+:/, "");
				// alert("getTermTex2t->"+term);
				return term;
			}
		}
	...
	$.Autocompleter.defaults = {
		...
		parse : function(data) {
			...
			for ( var i = 0; i < rows.length; i++) {
				...
				if (row) {
					...
					// filter by attributes of this sample group I need to put
					// fieldsNames.length=0 because
					// whem i'm using the autocomplete in the header I dont want
					// to make this filter
					// alert(fieldsNames);
					// alert(fieldsNames.length);
		...
		formatItem : function(data, pos, max, value, term) {
			...
			if ("f" == data.data[1]) {
				value = value + "<span class=\"ac_field\">Filter "
						+ data.data[2] + "</span>";
				...
				if (null != data.treeLevel) {
					if (data.treeId) {
						value = "<a href=\"javascript:void(0);\"><div class=\"ac_tree_control\"><div/></div></a>"
								+ value;
		...
		scroll : true,
		scrollHeight : 242
	// that is a really magic number, hehe :)
	};
	...
	$.Autocompleter.Cache = function(options) {
		...
		function populate() {
			...
			for ( var i = 0, ol = options.data.length; i < ol; i++) {
				...
				var value = options.formatItem(rawValue, i + 1,
						options.data.length);
				if (value === false)
					continue;
	...
	$.Autocompleter.Select = function(options, input, select, config) {
		...
		function init() {
			if (!needsInit)
				return;
			element = $(
					"<div class='ac_outer'><div class='ac_inner'></div></div>")
					.hide().addClass(options.resultsClass).css("position",
							"absolute").appendTo(document.body);

			list = $("<ul/>")
					.appendTo(element.find(".ac_inner"))
					.mouseover(
							function(event) {
		...
		// ok, for this first iteration we will abuse internal code in order to
		// process tree items
		function fillList() {
			list.empty();
			...
			for ( var i = 0; i < max; i++) {
				...
				var formatted = options.formatItem(data[i], i + 1, max, html,
						term);
				if (formatted === false)
					continue;
				var li = $("<li/>")
						...
						.addClass(
								data[i].treeId ? (data[i].treeIsExpanded ? "ac_tree_expanded"
										: "ac_tree_collapsed")
										: "").appendTo(list)[0];
		...
		return {
			...
			show : function() {
				...
				onWinResize();
				element.css({
					top : offset.top + input.offsetHeight + 2, // that looks
					// much cooler
					// visually
					left : offset.left
				}).show();
