    ...
    private void updateAlternativesMap(String term) {
        ...
        if (!isStopWord(loweredTerm)) {
            ...
            if (!alternatives.isEmpty()) {
                Set<String> existingAlternatives = getNameToAlternativesMap().get(loweredTerm);
                if (existingAlternatives == null) {
                    existingAlternatives = new HashSet<String>();
                    getNameToAlternativesMap().put(loweredTerm, existingAlternatives);
                }
                for (String alternativeTerm : alternatives) {
                    ...
                    Set<String> alternativesToAlternative = getNameToAlternativesMap().get(loweredAlternativeTerm);
                    if (alternativesToAlternative == null) {
                        alternativesToAlternative = new HashSet<String>();
                        getNameToAlternativesMap().put(loweredAlternativeTerm, alternativesToAlternative);
                    }
